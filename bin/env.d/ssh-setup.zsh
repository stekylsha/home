...
if [[ ${#P_SSH_AGENT_PID} -gt 1 ]]; then
    ...
    if [[ -z "${P_SSH_AUTH_SOCK}" ]]; then
        ...
        P_SSH_AUTH_SOCK=$( lsof -a -u ${LOGNAME} -U -p ${P_SSH_AGENT_PID[1]} | tail -n 1 | sed 's#^[^/]*##' )
    fi
    ...
    cat << EOF > ${HOME}/.SSH_AGENT
SSH_AUTH_SOCK=${P_SSH_AUTH_SOCK}; export SSH_AUTH_SOCK;
SSH_AGENT_PID=${P_SSH_AGENT_PID[1]}; export SSH_AGENT_PID;
EOF
...
unset P_SSH_AGENT_PID
unset P_SSH_AUTH_SOCK
